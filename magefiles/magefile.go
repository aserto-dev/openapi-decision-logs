...
var bufImage = "buf.build/aserto-dev/decision-logs"
...
func getClientFiles(fileSources string) ([]string, error) {
	...
	protoFiles, err := fsutil.Glob(filepath.Join(bufExportDir, "aserto", "decision-logs", "v2", "**", "*.proto"), excludePattern)
	if err != nil {
		return clientFiles, err
	}
...
func GenerateDev() error {
	...
	bufImage := filepath.Join(getProtoRepo(), "bin", "decision-logs.bin#format=bin")
...
func mergeOpenAPI() error {
	const (
		repo = "github.com/aserto-dev/openapi-decision-logs"
	)
	...
	services := []Service{
		{
			outfile: "service/decision-logs/openapi.json",
			subServices: []string{
				"aserto/decision-logs/v2/decision_logs.swagger.json",
			},
		},
		{
...
func publishOpenAPIv3(version string) error {
	...
	services := []Service{
		{
			input:      "service/decision-logs/openapi.json",
			config:     "config/decision-logs-config.json",
			jsonOutput: "publish/decision-logs/openapi.json",
			yamlOutput: "publish/decision-logs/openapi.yaml",
		},
	}
...
func generateEmbedCode() error {
	...
	targets := []string{
		"publish/decision-logs/openapi.go",
	}
